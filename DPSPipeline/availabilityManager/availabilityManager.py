...
class updateAvailability(QtCore.QThread):
	...
	#needs to just add phases that are in calendar range
	for pa in sharedDB.myPhaseAssignments.values():
	    if int(sharedDB.myProjects[str(pa._idprojects)]._idstatuses) < 3:
...
class AvailabilityManager(QObject):
    ...
    '''
    def AddBookingAtDate(self,date):
	
	bookingdict = {}
	
	#if booking dictionary has date key get value
	if date in self._availability:
	    bookingdict = self._availability[date]
	    
	book = booking.Booking()
	
	bookingdict[str(book.id())] = book
	
	self._availability[str(date)] = bookingdict
	
	return book
	    
   
    def RemoveBookingById(self, id):
	pass
