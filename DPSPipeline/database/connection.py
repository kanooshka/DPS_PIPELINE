...
class Connection(QObject):
	...
	def __init__(self,_user = '', _password = ''):
		...
		self._remote = 0
		self._autoUpdateFrequency = 3000
	...
	def CheckForNewEntries (self):
		...
		projrows = self.query("SELECT idprojects, name, due_date, idstatuses, renderWidth, renderHeight, description, folderLocation, fps FROM projects WHERE idstatuses != 4 AND idstatuses != 5 AND idstatuses != 6 AND timestamp > \""+str(sharedDB.lastUpdate)+"\"")
